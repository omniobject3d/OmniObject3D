...
def subprocess_fn(rank, c, temp_dir):
    ...
    if c.inference_vis and c.inference_mode=='normal':
        inference_3d.inference(rank=rank, **c)
    # Execute training loop.
    elif c.inference_vis and c.inference_mode=='competition':
        inference_3d.inference_for_oo3dcompetition(rank=rank, **c)
    else:
...
@click.option('--n_forimg', help='numbersample for image metric', metavar='INT', type=click.IntRange(min=1), default=5000,)
@click.option('--n_forgeo', help='numbersample for geometric metric', metavar='INT', type=click.IntRange(min=1), default=1000,)
@click.option('--inference_mode', help='The type of inference', type=str, default='competition')
...
def main(**kwargs):
    ...
    c.n_shape = opts.n_forimg
    c.n_mesh = opts.n_forgeo
    c.inference_mode = opts.inference_mode
    # Training set.
