...
from training.inference_utils import save_visualization_oo3d, save_geo_for_inference_simple_oo3d
...
def inference_for_oo3dcompetition(
        n_shape,
        n_mesh,
        run_dir='.',  # Output directory.
        training_set_kwargs={},
    ...
    # fixme: about how to gain the number of classes
  # subclass of training.dataset.Dataset
    training_set = dnnlib.util.construct_class_by_name(
        **training_set_kwargs)  # subclass of training.dataset.Dataset
    common_kwargs = dict(
        c_dim=training_set.label_dim, img_resolution=1024, img_channels=3)
    G_kwargs['device'] = device
    ...
    grid_z = torch.randn([n_shape, G.z_dim], device=device).split(1)  # random code for geometry
    grid_tex_z = torch.randn([n_shape, G.z_dim], device=device).split(1)  # random code for texture
    grid_c = torch.zeros([n_shape, G.c_dim], device=device).split(1)
    #ipdb.set_trace()
    # for i,c in enumerate(grid_c):
    #     c[:,i%G.c_dim] = 1.

    print('==> generate ')
    save_visualization_oo3d(
        G_ema, grid_z, grid_c, run_dir, 0, grid_size, 0,
        save_all=True,
        grid_tex_z=grid_tex_z
    )
    ...
    #print('==> generate 7500 shapes for evaluation')
    save_geo_for_inference_simple_oo3d(G_ema, run_dir, n_mesh)
