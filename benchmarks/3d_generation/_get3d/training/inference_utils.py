...
def save_visualization_oo3d(
        G_ema, grid_z, grid_c, run_dir, cur_nimg, grid_size, cur_tick,
        image_snapshot_ticks=50,
        save_gif_name=None,
        save_all=True,
        grid_tex_z=None, n_shape = 10, suffix='',
):
    '''
    ...
    with torch.no_grad():
        ...
        for i_camera, camera in enumerate(camera_list):
            ...
            mesh_f_list = []
            #ipdb.set_trace()
            for idx, (z, geo_z, c) in enumerate(zip(grid_tex_z, grid_z, grid_c)):
                ...
                #ipdb.set_trace()
                #os.makedirs(os.path.join(run_dir,str(idx)), exist_ok=True)
                # rgb_img = ((rgb_img +1) * (255 / (2))).cpu().numpy()
                # rgb_img = np.rint(rgb_img).clip(0, 255).astype(np.uint8)
                # imageio.imwrite(os.path.join(run_dir,str(idx),str(i_camera)+'.png'),np.uint8(rgb_img[0].transpose(2,1,0)))
                rgb_img=rgb_img.cpu().numpy()
                os.makedirs(os.path.join(run_dir,'results_img',str(idx)), exist_ok=True)
                save_image_grid(
                    rgb_img, os.path.join(run_dir,'results_img', str(idx),str(i_camera)+'.png'),
...
def save_geo_for_inference_simple_oo3d(G_ema, run_dir, n_gen = 1500 * 5):
    '''
    ...
    with torch.no_grad():
        ...
        truncation_phi = 1.0
        mesh_dir = os.path.join(run_dir, 'results_mesh')
