...
class ImageFolderDataset(Dataset):
    def __init__(
        ...
        if data_camera_mode == 'shapenet_car' or data_camera_mode == 'shapenet_chair' \
                ...
                or data_camera_mode == 'ts_animal' or data_camera_mode == 'oo3d':
            print('==> use {} dataset'.format(data_camera_mode))
            ...
            print('==> use folder number %s' % (len(folder_list)))
            ...
            if data_camera_mode != 'oo3d':
                ...
                split_data = dict()
                N = 0
                for i, cat_folder in enumerate(folder_list):
                    if not os.path.isdir(cat_folder):
                        continue
                    all_categories.append(cat_folder.split('/')[-1])
                    objects = sorted(os.listdir(cat_folder))
                    split_objs = []
                    for obj in objects:
                        N += 1
                        split_objs.append(obj)
                        rgb_list = sorted(os.listdir(os.path.join(cat_folder, obj)))
                        rgb_list = [n for n in rgb_list if n.endswith('.png') or n.endswith('.jpg')]
                        rgb_file_name_list = [os.path.join(cat_folder, obj, n) for n in rgb_list]
                        all_img_list.extend(rgb_file_name_list)
                        all_mask_list.extend(rgb_list) # fixme
                        oo3d_labels.extend([i] * len(rgb_list))
                    split_data.update({cat_folder.split('/')[-1]:split_objs})
                self.category_to_label = {cat:idx for idx, cat in enumerate(all_categories)}
                self.oo3d_labels = np.asarray(oo3d_labels).astype(np.int64)
                print(N, ' objects selected.')
        ...
        ''' debug '''
        # for fname in tqdm(self._all_fnames):
    ...
    def _load_raw_labels(self):
        # oo3d label
        if self.data_camera_mode == 'oo3d':
            return self.oo3d_labels
        else:
            raise None
