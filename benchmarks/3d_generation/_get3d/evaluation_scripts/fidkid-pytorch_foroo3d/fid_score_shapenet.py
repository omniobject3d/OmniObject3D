...
def compute_statistics_of_path(path, model, batch_size, dims, device,
                               num_workers=1,reso=128,dataset='oo3d'):

    basepath=os.path.join(path,'fid'+str(reso))
    ...
    try:
        ...
        m=np.load(os.path.join(basepath,'oo3d'+str(reso)+'mean.npy'))
        s=np.load(os.path.join(basepath,'oo3d'+str(reso)+'std.npy'))
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1,reso=128,dataset='oo3d'):
    ...
    m2, s2 = compute_statistics_of_path_new(paths[1], model, batch_size,
                                        dims, device, num_workers,reso=reso,dataset=dataset)
