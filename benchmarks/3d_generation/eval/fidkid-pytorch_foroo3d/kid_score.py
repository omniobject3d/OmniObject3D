...
def _compute_activations(path, model, batch_size, dims, cuda, model_type,reso,dataset):
    ...
    #files=files[:2]
    basepath=os.path.join(path,'kid'+str(reso))
    os.makedirs(os.path.join(basepath), exist_ok=True)
    #ipdb.set_trace()
    if model_type == 'inception':
        if os.path.exists(os.path.join(basepath,'oo3d'+str(reso)+'kid.npy')):
            act=np.load(os.path.join(basepath,'oo3d'+str(reso)+'kid.npy'))
...
if __name__ == '__main__':
    ...
    results = calculate_kid_given_paths(paths, args.batch_size,False , args.dims, model_type=args.model,reso=args.reso,dataset=args.dataset)
