...
def train(args):
    ...
    train_dataset, train_sampler = create_training_dataset(args, mode='train')
    ...
    val_dataset, val_sampler = create_training_dataset(args, mode='val')
...
def log_view_to_tb(writer, global_step, args, model, ray_sampler, projector, gt_img,
    ...
    savedir = os.path.join(args.rootdir, 'out', args.expname, 'visual')
    os.makedirs(savedir, exist_ok=True)

    cv2.imwrite(os.path.join(savedir, 'ref_'+str(global_step)+'.png'), (np.concatenate((pred_rgb.numpy()[...,::-1], gt_img.numpy()[...,::-1]), axis=1) * 255.).astype(np.uint8))
