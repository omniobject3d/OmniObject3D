...
class MVSSystem(LightningModule):
    ...
    def forward(self, dataset, savedir):
        self.MVSNet.train()
        num_views = 10
        image_preds_all = []
        ...
        for idx in range(num_views):
            ...
            with torch.no_grad():
                ...
                imageio.imwrite(os.path.join(savedir, file_name), (image_preds.numpy()*255.).astype(np.uint8))
...
if __name__ == '__main__':
    ...
    if args.validate_mesh:
        ...
        with torch.no_grad():
            validate_mesh(savedir=testsavedir, **system.render_kwargs_train) # , scale_mats_np=scale_mats_np